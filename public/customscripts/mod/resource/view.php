...
// ========== WATERMARK FORCE ==========
// Build the original pluginfile.php URL.
$path = '/'.$context->id.'/mod_resource/content/'.$resource->revision.$file->get_filepath().$file->get_filename();
$originalurl = moodle_url::make_file_url('/pluginfile.php', $path, $displaytype == RESOURCELIB_DISPLAY_DOWNLOAD);
$originalurl = $originalurl->out(false); // Absolute URL.

// Convert it to watermarked URL.
$watermarkedurl = new moodle_url('/local/watermark/download.php', ['file' => urlencode($originalurl)]);
$watermarkedurl = $watermarkedurl->out(false);
// =====================================

if ($redirect && !$forceview) {
    // Redirect to the watermarked version instead of pluginfile.php.
    redirect($watermarkedurl);
}

switch ($displaytype) {
    case RESOURCELIB_DISPLAY_EMBED:
        // For embed, we must replace the file URL with the watermarked one.
        // resource_display_embed uses the $file object; we cannot easily replace it there.
        // So we override by capturing output or using a custom function.
        // Simpler: print the embed code with the watermarked URL.
    ...
    case RESOURCELIB_DISPLAY_FRAME:
        // For frame, we also use the watermarked URL.
        ...
        echo '<iframe id="resourceframe" src="' . s($frameurl) . '" width="100%" height="600" title="' . s($title) . '"></iframe>';
        echo '</div>';
        echo $OUTPUT->footer();
        break;

    default:
        // For workaround (e.g., forced download), also use watermarked URL.
        // resource_print_workaround expects a file object; we'll override by redirecting.
        redirect($watermarkedurl);
        break;
}